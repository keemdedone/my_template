    ...
    if (!ticking) {
      requestAnimationFrame(() => {
        const imageToReorder = [...slideshow.children].reduce((prev, current) =>
          ~direction
            ? Number(prev.style.order) < Number(current.style.order)
              ? prev
              : current
            : Number(prev.style.order) > Number(current.style.order)
            ? prev
        ...
        if (~direction && slideshow.scrollLeft > slideshow.scrollWidth / 2) {
          ...
          for (const image of [...slideshow.children]) {
            if (image == imageToReorder)
              image.style.order = slideshow.children.length - 1;
        ...
        // Scrolling left
          ...
          !~direction &&
          slideshow.scrollLeft + slideshow.clientWidth <
            slideshow.scrollWidth / 2
        ) {
          slideshow.scrollLeft = slideshow.scrollLeft + imageToReorder.width;
